...
- **`src/banter/`** - CALDERA FM, the in-browser AI radio DJ (2026-07-14 spike, `CONFIG.radioDj`, default OFF). Magma Max's lines are generated live by Gemma 4 E2B (ONNX q4f16) via transformers.js on WebGPU in a Web Worker, reacting to real telemetry (tricks ring, store, impact edges). `director.ts` owns scheduling: event priority + HEAT tone tiers (mild/solid/wild), cooldowns, and a frame-health EMA gate - when frames and banter collide, banter loses. `gate.ts` screens every line for kid-appropriateness before render. No WebGPU → feature silently absent. First enable downloads ~3.1GB (browser-cached; ~3.6s warm loads). Measured: 208 back-to-back generations hold 60fps vsync-locked at 1.14ms avg frame cost. Findings + open directions: `research/npc-banter-findings.md`.
