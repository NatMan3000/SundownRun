...
| World map | `bun run map` -> `World Map.html` (Windows: `Sundown Run Map Gen.bat` - regenerates + opens it) | Interactive top-down map generated FROM the game code (terrain exports: CIRCUIT/JUMPS/BANKS/PLAYGROUNDS + heightfield render); clickable markers explain each feature and point at the code that makes it. Regenerate + commit after any world change. Shard positions are hand-mirrored from Delights.tsx buildShards() - update both. |
